...
public class VerbPhraseTest extends SimpleNLG4Test {
    ...
    public void testVerbParticle() {
        VPPhraseSpec v = this.phraseFactory.createVerbPhrase("fall down");

        assertEquals("down", v.getFeatureAsString(Feature.PARTICLE));
        assertEquals("fall", ((WordElement) v.getVerb()).getBaseForm());
        ...
        assertEquals("fell down", this.realiser.realise(v).getRealisation());

        v.setFeature(Feature.FORM, Form.PAST_PARTICIPLE);
        assertEquals("fallen down", this.realiser.realise(v).getRealisation());
    }
    ...
    public void simplePastTest() {
        ...
        assertEquals("fell down", this.realiser.realise(this.fallDown).getRealisation());
    }
    ...
    public void tenseAspectTest() {
        ...
        assertEquals("had fallen down", this.realiser.realise(this.fallDown).getRealisation());

        // had been falling down
        this.fallDown.setFeature(Feature.PROGRESSIVE, true);
        assertEquals("had been falling down", this.realiser.realise(this.fallDown).getRealisation());
        ...
        assertEquals("will have been kicked", this.realiser.realise(this.kick).getRealisation());

        // will have been being kicked
        this.kick.setFeature(Feature.PROGRESSIVE, true);
        assertEquals("will have been being kicked", this.realiser.realise(this.kick).getRealisation());

        // will not have been being kicked
        this.kick.setFeature(Feature.NEGATED, true);
        assertEquals("will not have been being kicked", this.realiser.realise(this.kick).getRealisation());
        ...
        assertEquals("will not have been kicking the man", this.realiser.realise(this.kick).getRealisation());
        ...
        assertEquals("has not been kicking the man", this.realiser.realise(this.kick).getRealisation());
    }
    ...
    public void complementationTest() {
        // was kissing Mary
        PhraseElement mary = this.phraseFactory.createNounPhrase("Mary");
        ...
        assertEquals("was kissing Mary", this.realiser.realise(this.kiss).getRealisation());
        ...
        assertEquals("was kissing Mary and Susan", this.realiser.realise(this.kiss).getRealisation());
        ...
        assertEquals("was being kissed", this.realiser.realise(this.kiss).getRealisation());

        // make it plural (this is usually taken care of in SPhraseSpec)
        this.kiss.setFeature(Feature.NUMBER, NumberAgreement.PLURAL);
        assertEquals("were being kissed", this.realiser.realise(this.kiss).getRealisation());
        ...
        assertEquals("was kissing Mary and Susan in the room", this.realiser.realise(this.kiss).getRealisation());

        // passivise again: should make direct object disappear, but not postMod
        // ="was being kissed in the room"
        this.kiss.setFeature(Feature.PASSIVE, true);
        this.kiss.setFeature(Feature.NUMBER, NumberAgreement.PLURAL);
        assertEquals("were being kissed in the room", this.realiser.realise(this.kiss).getRealisation());
    }
    ...
    public void clausalComplementTest() {
        ...
        s.setSubject(this.phraseFactory.createNounPhrase("John"));
        ...
        assertEquals("John was kissing Mary and Susan in the room", this.realiser.realise(s).getRealisation());

        // make the main VP past
        this.say.setFeature(Feature.TENSE, Tense.PAST);
        assertEquals("said", this.realiser.realise(this.say).getRealisation());
        ...
        PhraseElement s2 = this.phraseFactory.createClause(
                this.phraseFactory.createNounPhrase("all"),
                "be",
                this.phraseFactory.createAdjectivePhrase("fine")
        );

        s2.setFeature(Feature.TENSE, Tense.FUTURE);
        assertEquals("all will be fine", this.realiser.realise(s2).getRealisation());
    ...
    public void coordinationTest() {
        ...
        assertEquals(
                "could not have been kissing the dog and "
